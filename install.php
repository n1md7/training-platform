...
try {
    ...
    $query = $dbh->prepare("select count(*) as count from $dbut where 1");
    $query->execute();
    $result = $query->fetch(PDO::FETCH_ASSOC);
    
    if($result && $result['count'] > 0) {
        die("Already migrated. Check the app.\n");
    }
...
if (isset($_POST['install'])) {
    try {
        ...
        if (file_put_contents($path_to_file, $file_contents) === false) {
            $dir = __DIR__;
            echo "Ups! We don't have a permission to change content of config.php file!\n";
            echo "Open config.php and change credentials manually\n";
        } else {
            echo "config.php file modified successfully.\n";
        }
